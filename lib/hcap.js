...
var exist = fs.existsSync(path.join(__dirname, '../cap_img/'));
if(!exist) {
	fs.mkdirSync(path.join(__dirname, '../cap_img/'));
	fs.mkdirSync(path.join(__dirname, '../cap_img/0'));
}
...
var img_path = path.join(__dirname, '../cap_img/', ( cluster.isWorker ? cluster.worker.id : '0'));
var pid = process.pid;
